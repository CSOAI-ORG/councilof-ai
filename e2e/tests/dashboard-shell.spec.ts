...
test("sidebar exposes the ten primary user jobs as direct /dashboard?tab= links", async ({
  ...
  await expect(page.getByRole("link", { name: "All tools", exact: true })).toHaveAttribute(
    "href",
...
test("the board pane quotes GET /api/gspc and embeds the living Space — nothing typed", async ({
  ...
  expect(
    cardCount + emptyState,
    "axis cards mounted (live) or honest empty-state",
  ).toBeGreaterThanOrEqual(1);
});

test("legacy /os and /gspc-scoreboard doors land inside the Dashboard", async ({
  page,
}) => {
...
test("one workspace keeps the composer and workspace rail access while a tool pane is open", async ({
  ...
  await expect(page.getByLabel("Open account and workspace menu")).toHaveCount(
    isMobile ? 1 : 2,
  );
});

test("embedded dashboard uses the same workspace instead of the retired Council OS", async ({
  page,
}) => {
  ...
  await expect(
    page.locator('[data-testid="dashboard-shell"]'),
    "embedded mode omits the outer navigation shell",
  ).toHaveCount(0);
  await expect(
    page.getByLabel("Open Council OS"),
...
test("a question containing 'show' stays in chat while an explicit command navigates", async ({
  page,
}) => {
  await openTab(page, "home");
  const composer = page.getByLabel("Ask the Council, or name a pane to open");

  await composer.fill("What does the published safety measurement show?");
  await page.getByRole("button", { name: "Ask", exact: true }).click();
  await expect(
    page
      .getByRole("log", { name: "Council of AI conversation" })
      .getByText("What does the published safety measurement show?", {
        exact: true,
      }),
  ).toBeVisible();
...
test("GSPC quests are a styled in-workspace game and never promote play into measurement", async ({
  ...
  await expect(page.locator("#grid .q")).toHaveCount(12);
  await expect(page.getByText(/signed measurement/i)).toHaveCount(0);
...
test("the 22-axis learning arena keeps coaching, practice and human review in one workspace", async ({
  ...
  await expect(pane.locator("[data-axis-learning]")).toHaveCount(22);
  await expect(pane.getByTestId("learning-progress")).toContainText("0 of 5");
  ...
  await expect(pane.getByTestId("learning-progress")).toContainText("1 of 5");
