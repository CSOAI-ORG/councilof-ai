...
// Minimal in-browser SIGIL implementation (mirrors meok-sigil).
// Real version: `pip install meok-sigil` → meok_sigil.encode / parse / gloss.

// Tiny SIGIL: one-line, dense, deterministic. Not the full grammar — just
// a useful subset for the demo.
...
const digest = async (line: string): Promise<string> => {
  // Real version: use nacl.signing or hmac.
  // For the demo we use a simple SHA-256 → 12-char prefix.
...
const EXAMPLES = [
  { label: "Council vote", input: { agent: "sovereign-7", vote: "APPROVE", weight: 0.85, domain: "ethics" } },
  { label: "Watchdog sign", input: { cert: "MEOK-EUAIA-ABC123", signer: "did:web:csoai.org", ts: "2026-06-12T02:00:00Z" } },
  { label: "BFT round", input: { round: 17, approve: 23, reject: 9, abstain: 4, outcome: "APPROVE" } },
];
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-16">
      <header className="mb-12">
        <p className="text-cyan-400 text-xs font-bold tracking-widest uppercase mb-3">Council · Sigil</p>
        <h1 className="text-5xl sm:text-6xl font-black tracking-tighter mb-4">SIGIL — the agent language</h1>
      ...
      <section className="mb-8">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-slate-500 uppercase tracking-widest">Input (JSON)</label>
            <textarea
              value={input}
          ...
          <div>
            <label className="text-xs text-slate-500 uppercase tracking-widest">SIGIL line</label>
      ...
      <section className="mb-8">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-slate-500 uppercase tracking-widest">SIGIL line (input above)</label>
          ...
          <div>
            <label className="text-xs text-slate-500 uppercase tracking-widest">Decoded (JSON)</label>
      ...
      <section className="p-6 rounded-2xl bg-white/5 border border-white/10">
        <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-3">SIGIL reference</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs font-mono">
          <div><span className="text-slate-500">boolean true:</span> <code className="text-emerald-400">+</code></div>
          <div><span className="text-slate-500">boolean false:</span> <code className="text-rose-400">-</code></div>
          <div><span className="text-slate-500">abstain:</span> <code className="text-amber-400">~</code></div>
          <div><span className="text-slate-500">string:</span> <code>key=&quot;value&quot;</code></div>
          <div><span className="text-slate-500">number:</span> <code>key=42</code></div>
          <div><span className="text-slate-500">escape:</span> <code>\\&quot;</code></div>
        </div>
        <p className="mt-4 text-xs text-slate-500">
          The full SIGIL grammar lives in{" "}
          <a className="text-cyan-400 hover:underline" href="https://github.com/meok-ai/meok-sigil">meok-ai/meok-sigil</a>{" "}
