...
export default function CataloguePage() {
  ...
  return (
    <div className="min-h-screen">
      <div className="bg-accent/30 border-b border-border py-24 relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('/grid.svg')] bg-center [mask-image:linear-gradient(180deg,white,rgba(255,255,255,0))] opacity-20"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-3xl mx-auto space-y-6">
            <h1 className="text-4xl md:text-6xl font-black text-foreground">Agent Catalogue</h1>
            ...
            <div className="relative max-w-2xl mx-auto mt-8">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-4 bg-card border border-border rounded-2xl focus:ring-2 focus:ring-brand-500 focus:border-brand-500 text-foreground placeholder-slate-500 backdrop-blur-xl"
              />
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-6">
          {AGENTS.map((agent) => (
            <div key={agent.name} className="bg-card border border-border rounded-3xl p-8 hover:border-brand-500/50 transition-all group backdrop-blur-sm relative overflow-hidden">
              ...
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground font-mono">{agent.provider}</p>
                  </div>
                </div>
              ...
              <p className="text-foreground/80 text-sm leading-relaxed mb-6 h-10 line-clamp-2">
                {agent.description}
              </p>

              <div className="flex flex-wrap gap-2 mb-8">
                {agent.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 rounded-full bg-muted text-foreground/80 text-xs font-medium border border-border/60">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <div className="flex items-center gap-4 pt-6 border-t border-border">
                <button className="flex-1 py-3 bg-brand-600 hover:bg-brand-500 text-foreground rounded-xl font-semibold transition-colors flex items-center justify-center gap-2">
                  <Zap className="w-4 h-4" /> Deploy via OS
                </button>
                <button className="px-6 py-3 bg-muted hover:bg-muted/80 text-foreground/80 rounded-xl font-medium transition-colors border border-border/60">
                  View Source
                </button>
              </div>
            </div>
