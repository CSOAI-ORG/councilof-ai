# packages/ — one grammar, not one blended score

Councilof-ai is already the master monorepo shape (`harness/`, `evidence/`, `mcp/`, `actions/`, `client/`). Sibling repos stay **donors** until a subtree lands. This directory is the sealed, signed, published surface for the pieces that were doctrine only.

| Package | Status tonight | Honesty |
|---|---|---|
| `trace` | software stub | Hardware / silicon RATS fields are **UNCHECKABLE**. Linux Foundation TRACE (25 Aug 2026) is not implemented here. |
| `aibom` | emitter stub | No CycloneDX/SPDX bytes until a lineage is passed in. BOM hash folds into a card when emitted. |
...
Live board is GET `/api/gspc`. TRACE/AIBOM stubs do not add a 23rd axis.
