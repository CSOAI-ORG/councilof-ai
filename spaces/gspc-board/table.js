...
const READ = [
  {
    ...
    body: "A Hub id, a library card, a hosted name. We walk metadata — repo, revision, licence, files — without downloading weights. That listing is DISCOVERED. It is not a GSPC cell.",
  },
  {
    title: "A measurement is a signed cell",
    body: "One unique lineage. A frozen bank. n, a figure, an interval, a separation. Quotable only after the instrument is bolted and the card is signed. That is the grade — and only then.",
  },
  {
...
const CENSUS_SITES = [
  { id: "huggingface", title: "Hugging Face Hub", status: "planted", does: "Metadata walk of every listed model — id, revision, licence, files. More than two million repos. No weight download. Listing is DISCOVERED." },
  { id: "openrouter", title: "OpenRouter", status: "next", does: "Hosted ids as a public catalogue. Not a measurement target until a unique lineage is run." },
  { id: "ollama", title: "Ollama library", status: "next", does: "Local pull list as DISCOVERED. A library card is not a signed cell." },
...
function openAxis(name) {
  ...
  if (!recs.length) {
    document.getElementById("rec-table").innerHTML =
      ...
      `<tbody><tr><td colspan="2">${on ? "No compact cards listed for this axis yet. The cell on the board is still the finding." : "Empty slot — no signed cells."}</td></tr></tbody>`;
  } else {
    document.getElementById("rec-table").innerHTML =
      ...
      (recs.length > 12 ? `<tr><td colspan="3">${recs.length} records in the public index.</td></tr>` : "") +
      `</tbody>`;
  }
...
function renderHealth() {
  ...
  const rowsH = [
    ...
    ["Items in banks", "totals.items — rows behind MEASURED axes", String(t.items ?? "—")],
    ["Signed cards", "signed/card_index.json", String(INDEX.length || "—")],
    ["Verify pass", "/gspc-verify · verify_card", "open"],
...
async function renderQueue() {
  try {
    ...
    document.getElementById("census-counts").innerHTML = [
      [q.n, "Named in queue", "DISCOVERED · still UNMEASURED"],
      [q.n_measured, "Measured in this queue", "stays 0 until a signed cell exists"],
      [q.n_site_pages ?? 0, "Queue pages", "pagination of the listing walk"],
      [">2,000,000", "Hub scale", "model listings — not measured lineages"],
...
function renderHonesty() {
  ...
  document.getElementById("lane-note").textContent = names
