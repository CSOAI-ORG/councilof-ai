# AILuminate → GSPC importer specification v0.2

**Document class:** interop specification. **Status:** draft-implemented-doctrine · **Date:** 2026-09-12 (v0.2: crosswalk joins cut to the defensible 8 after the #1876 review).
...
- `attachment_to_axes` is `safety` at most (chat hazard ↔ calibrated-refusal
  instrument), and only for the 8 of 14 codes in the harmful-request family
  (`vcr ncr src cse ssh iwp hte sxc_prn`). The remaining 6 codes (`ipv`, `dfm`,
  `prv`, `spc_ele/fin/hlt`) carry **no join** — no GSPC axis measures IP
  regurgitation, defamation, privacy leakage or advice quality, and joining
  them anyway was the class of error review rejected in PR #1876. Gaps are
  recorded as gaps, never projected.
