...
3. **Stored:** `sha256` of the response bytes, `sha256` of the normalised text, byte length, HTTP status, `etag`, `last-modified`, `content-type`, final URL, per-target request-start `fetched_at`, robots decision, state, reason. `run_at` separately identifies the start of the batch. **Not stored:** the content. Not one line, not a snippet, not a title. The test suite asserts that fixture text never appears in the state file or any leaf.
