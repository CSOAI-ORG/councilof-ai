...
5. **Cursor "Codebase" = Cursor Origin**, a paid-plan-only, early-beta code host launched 17 Aug 2026. It is not a candidate (see §5).
...
| **Cursor Origin** (`cursor.com/codebase/*`) | ❌ no first-party CI (Depot/Buildkite integrations run GH-Actions YAML — third-party, paid); no cron; no OIDC | ❌ | ❌ **no public repos in beta (?)**; URL redirects to WorkOS login | ❌ | $20+/mo per seat (paid plans only) | ❌ | not a candidate (§5) |
...
## 5. Cursor "Codebase" verdict

- `https://cursor.com/codebase` does not render publicly: it 307-redirects to `cursor.com/api/auth/login` → WorkOS AuthKit. It is the logged-in **Codebase tab** of **Cursor Origin**, Cursor's code-hosting product announced 17 Aug 2026: "Cursor can now host your code"; "rolling out today in early beta on all paid plans"; repos, pull requests, code browsing, GitHub sync; CLI clone/push; repo URLs `cursor.com/codebase/{name}`. — https://cursor.com/changelog/origin-code-hosting
- For synced repos "GitHub stays the source of truth: pushes keep going to GitHub, and Origin mirrors the result" — i.e. it does not remove the GitHub dependency, it adds one.
- CI: "Connect Depot or Buildkite. Both run your existing GitHub Actions workflows" — no first-party CI, no cron, no OIDC issuer, no secrets store of its own documented.
- Public repositories: not offered in the beta per multiple third-party reviews (Appwrite, DigitalOcean, SitePoint, Aug 2026) — **UNVERIFIED** against Cursor's own page, which is silent; the login wall on the URL is consistent with no anonymous access.
- Cost: Pro $20/mo minimum; Hobby (free) excluded.
- **Verdict: not a code home.** Fails R1, R2, R3, R4 and R6 today. It may become a useful *agent workspace* mirror later (Origin's stated direction is "agent-native features"), which is a different question from where the bytes live. Re-evaluate when it has public repos, first-party CI and a stable GA — not before.
...
- Cursor Origin changelog — https://cursor.com/changelog/origin-code-hosting ; `cursor.com/codebase` → 307 to `cursor.com/api/auth/login` → WorkOS; reviews: https://appwrite.io/blog/post/cursor-origin-review-an-engineers-perspective , https://www.digitalocean.com/resources/articles/cursor-origin-vs-github , https://www.sitepoint.com/cursor-origin-github-sync-code-hosting-beta/ , https://siliconangle.com/2026/08/17/cursor-launches-origin-code-hosting-service-to-compete-with-github/
