...
| MCP server (stdio) | npm `csoai-gspc-mcp` | published | REAL | Reads the same two definitions files as the HTTP door. Payment is the `x_payment` argument, not a transport header, so carrying a paid tool is a packaging choice: 0.1.1 shipped the seven free tools, 0.2.0 ships all twelve. Ask `npm view csoai-gspc-mcp version` rather than trusting a version written here. |
