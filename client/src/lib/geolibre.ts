 ...
 *
 * Law: geo-location is OFF by default. Prefer first-party GET /api/geo-hint
 * (Cloudflare edge country — no third-party IP API). Manual region pick never
 * touches the network. IP/country is a proxy default, not a legal fact.
 */
...
/** Explicit opt-in. Prefer first-party /api/geo-hint (CF edge country) — never ipapi. */
export async function enableGeo(): Promise<void> {
  ...
  try {
    ...
    const regionFromHint =
      d.region === "EU" || d.region === "UK" || d.region === "US" || d.region === "CN"
        ? d.region === "UK"
          ? "UK"
          : d.region
        : iso2ToRegion(iso2);
    setState({
      source: "ip",
      countryIso2: iso2,
      regionCode: REGIONS[regionFromHint] ? regionFromHint : iso2ToRegion(iso2),
      resolving: false,
    });
  } catch {
