...
function requestDeadline(caller: AbortSignal | undefined, timeoutMs: number) {
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    throw new Error("MCP timeoutMs must be a positive finite number");
  }
    ...
    for (const endpoint of endpoints) {
      try {
        const response = await fetch(
          endpoint,
          request(method, params, id, deadline.signal),
        );
        if (!response.ok) {
        ...
        // Caller cancellation and the internal deadline both stop fallback.
        // In particular, a timed-out tools/call is never replayed elsewhere.
