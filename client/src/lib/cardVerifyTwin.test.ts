/**
 * cardVerifyTwin — the BROWSER twin against the PUBLISHED original.
 *
 * Split out of cardVerify.test.ts during the 2026-08-27 consolidation. Two lanes had
 * written a file of that name for different subjects: master's tests the SHARED
 * functions/_lib/cardVerify against the real published corpus (the SCITT-audit
 * regression), and lane/os-tools-real's — this one — proves client/src/lib/cardVerify
 * and public/signed/verify-card.mjs cannot drift apart. Neither is redundant, so
...
// run under vitest.
// Plain .mjs with no type declarations, deliberately dependency-free. This tsconfig
// resolves it via allowJs, so no suppression directive is needed here — and an unused
// one would itself be reported as an error.
...
describe("cardVerify — the browser twin of public/signed/verify-card.mjs", () => {
