...
function langDirective(): string {
  try {
    ...
    return LANG_NAMES[l]
      ? ` Respond in ${LANG_NAMES[l]}. Keep regulation names (EU AI Act, NIST, ISO 42001, DORA, NIS2) and any structured labels (e.g. RISK TIER, WHY, OBLIGATIONS) in English; translate the surrounding prose.`
      : "";
  } catch {
    return "";
  }
...
function isWrongPenalty(t: string): boolean {
  return (
    PENALTY_CTX.test(t) &&
    WRONG_FIG.test(t) &&
    !/(35\s*(million|m\b)|7\s?%|15\s*(million|m\b)|3\s?%)/.test(t)
  );
}
...
export async function askSovereign(
  ...
  const fallback =
    (opts && opts.fallback) ||
    "I can only speak as the CSOAI Council assistant on AI governance, regulation and cybersecurity — ask me about a framework, a system, or how to get compliant and I'll help.";
  ...
  try {
    ...
    if (r.ok) {
      ...
      const candidate =
        d && (d.answer ?? d.reply ?? d.response ?? d.message?.content);
  ...
  emitCard({
    kind: "dock-ask",
    summary: q.slice(0, 120),
    detail: "gateway unreachable or response rejected — clean fallback served",
    latencyMs: Date.now() - started,
    axis: "governance",
    source: "local-sim",
  });
