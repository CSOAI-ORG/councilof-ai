...
const CATEGORIES =
  (registry.categories as { name: string; count: number }[]) || [];
const FRAMEWORKS =
  (registry.frameworkCounts as { name: string; count: number }[]) || [];

export default function MCPRegistry() {
  useEffect(() => {
    document.title = `MCP Fleet — ${registry.total}+ AI governance tools | Council of AI`;
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return ALL_SERVERS.filter((s) => {
      ...
      if (activeFramework !== "All" && !fw.includes(activeFramework))
        return false;
      if (
        q &&
        !`${s.name} ${s.description} ${s.slug}`.toLowerCase().includes(q)
      )
        return false;
      return true;
    });
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-rose-600 text-white text-sm">
        <div className="container max-w-6xl py-2.5 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-center">
          <span className="font-semibold">
            ⏱ EU AI Act GPAI obligations are live (2 Aug 2026).
          </span>
          <span className="text-rose-100">
            Get audit-ready with the fleet —
          </span>
      ...
      <div className="bg-gradient-to-br from-white via-emerald-50 to-emerald-100 text-gray-900 py-20">
        <div className="container max-w-5xl">
          <Badge className="mb-6 bg-emerald-500/20 text-emerald-700 border-emerald-500/30">
            A2A Substrate
          </Badge>
          <h1 className="text-4xl md:text-5xl font-bold mb-6 leading-tight">
            The <span className="text-emerald-600">{registry.total}-MCP</span>{" "}
            Governance Fleet
          </h1>
          <p className="text-xl text-gray-700 leading-relaxed max-w-3xl mb-8">
            Every CSOAI tool is powered by Model Context Protocol servers —
            production compliance, safety and agent-infrastructure tools that
            any AI agent or human can call. Each one emits signed, auditable
            evidence mapped to the major global frameworks.
          </p>
          ...
          <div className="flex flex-wrap gap-3 mt-8">
            <Link href="/signup?source=mcp-api-key">
              ...
              >
                <KeyRound className="mr-2 h-4 w-4" aria-hidden="true" /> Get a
                free API key
              </Button>
            </Link>
            <Link href="/api-docs">
          ...
          <div className="grid sm:grid-cols-3 gap-4 text-sm text-gray-700">
            <div className="rounded-lg border border-emerald-100 bg-emerald-50/50 p-4">
              ...
              <p className="mt-1 text-gray-600">
                Reachable servers come from{" "}
                <code className="text-xs">/api/mcp</code> probes — never summed
                with directory-only ids.
              </p>
            </div>
            <div className="rounded-lg border border-emerald-100 bg-emerald-50/50 p-4">
              <p className="font-semibold text-emerald-800">Signer pin</p>
              <p className="mt-1 text-gray-600">
                Cards verify against{" "}
                <a
                ...
                — an unpublished key fails even when signature bytes parse.
              </p>
            </div>
            <div className="rounded-lg border border-emerald-100 bg-emerald-50/50 p-4">
              <p className="font-semibold text-emerald-800">Production MCP</p>
              <p className="mt-1 text-gray-600">
                <code className="text-xs">POST /mcp</code> exposes measure ·
                verify · jail-probe · enter-arena for Cursor, Claude, and other
                agents.
              </p>
            </div>
          </div>
      ...
      <div className="container max-w-6xl py-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((s) => (
            <Card
              key={s.slug}
              className="p-5 flex flex-col border hover:border-emerald-400 hover:shadow-md transition-all"
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <Link href={`/mcps/${s.slug}`}>
                  <h3 className="font-bold text-base leading-tight hover:text-emerald-600 cursor-pointer">
                    {s.name}
                  </h3>
                </Link>
                {s.builtInHouse && (
      ...
      <div className="bg-gradient-to-br from-slate-900 to-emerald-900 text-white py-20">
        <div className="container max-w-4xl text-center">
          <h2 className="text-4xl font-bold mb-6">
            Put the full fleet to work
          </h2>
          <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto">
            Connect your AI agents and compliance teams to all {registry.total}{" "}
            MCP tools through one gated endpoint (
            <code className="text-emerald-300">api.meok.ai</code>) — with
            bearer-token auth, per-user audit trails and signed,
            framework-mapped evidence on every call.
          </p>
          <div className="grid sm:grid-cols-2 gap-4 max-w-xl mx-auto mb-8 text-left">
            <div className="bg-white/10 rounded-xl p-5 border border-white/10">
              ...
              <p className="text-sm text-gray-300 mt-1">
                Every governance tool + EU AI Act tracking, signed evidence on
                every call
              </p>
            </div>
            <div className="bg-white/10 rounded-xl p-5 border border-white/10">
              ...
              <p className="text-sm text-gray-300 mt-1">
                Custom dev + SLA + dedicated support
              </p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="/contact">
              <Button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white"
              >
                Book a free 15-min EU AI Act diagnostic{" "}
