...
type Tier = { name: string; price: string; sub: string; tag?: string; cta: string; href: string; feats: string[]; highlight?: boolean };
    ...
    <div className="min-h-screen bg-[#03110b] text-emerald-50">
      ...
      <section className="mx-auto max-w-6xl px-6 pb-10">
        <div className="grid gap-5 lg:grid-cols-4 sm:grid-cols-2">
          {tiers.map((t) => (
            <div key={t.name} className={"flex flex-col rounded-2xl border p-5 " + (t.highlight ? "border-emerald-400/60 bg-emerald-500/5 shadow-[0_0_40px_-12px_rgba(16,185,129,.5)]" : "border-emerald-500/20 bg-[#05140d]")}>
              {t.tag && <span className="self-start rounded-full bg-emerald-500/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-emerald-300">{t.tag}</span>}
              ...
              <a href={t.href} className={"mt-5 rounded-xl px-4 py-2 text-center text-sm font-bold " + (t.highlight ? "bg-emerald-500 text-[#03110b] hover:bg-emerald-400" : "border border-emerald-400/40 text-emerald-100 hover:bg-white/5")}>{t.cta}</a>
            </div>
          ))}
        </div>
      </section>
