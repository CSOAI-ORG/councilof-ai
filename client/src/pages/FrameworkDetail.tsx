...
export default function FrameworkDetail() {
  useEffect(() => {
    document.title = "Framework measurement crosswalk | Council of AI";
  }, []);
  ...
  const tools = useMemo(
    () => (tag ? ALL.filter((s) => s.frameworks.includes(tag)) : []),
    [tag],
  );
  ...
  const schema = {
    ...
    "@type": "TechArticle",
    headline: `Council of AI × ${fw.name} measurement crosswalk`,
        ...
        <div className="bg-rose-600 text-white text-sm">
          <div className="container max-w-5xl py-2.5 flex flex-wrap items-center justify-center gap-x-3 text-center">
            <span className="font-semibold">
              ⏱ {fw.name} is binding — effective {fw.effective}.
            </span>
      ...
      <div className="bg-gradient-to-br from-blue-50 via-white to-emerald-50 border-b">
        <div className="container max-w-5xl py-14">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            >
              {fw.binding ? "Binding" : "Voluntary"}
            </Badge>
            <Badge variant="outline">
              Phase {fw.phase} · {fw.phaseLabel}
            </Badge>
          </div>
      ...
      <div className="container max-w-5xl py-12">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-bold text-lg mb-1">
                Mapped to the CSOAI 52-Article Charter
              </h2>
              <p className="text-gray-600 text-sm leading-relaxed">
                This crosswalk shows how {fw.name}'s requirements map to the
                CSOAI Partnership Charter, where the gaps are, and which MEOK
                MCPs satisfy each clause. Free under CC BY 4.0. See the full{" "}
                <Link href="/crosswalks">
          ...
          <div className="mb-12">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl font-bold flex items-center gap-2">
                <Boxes className="h-6 w-6 text-emerald-600" /> {tools.length}{" "}
                MCP tools that satisfy {fw.name}
              </h2>
              <Link href={`/mcps?framework=${encodeURIComponent(tag)}`}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {tools.slice(0, 9).map((t) => (
                <Link key={t.slug} href={`/mcps/${t.slug}`}>
        ...
        <Card className="p-8 bg-gradient-to-br from-slate-900 to-emerald-900 text-white text-center">
          ...
          <p className="text-gray-300 mb-6 max-w-2xl mx-auto">
            Book a free 15-minute diagnostic and we'll map your AI systems to{" "}
            {fw.name} — then put the MCP fleet to work generating audit-ready
            evidence.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            {fw.pdfName && (
              <a
                href={`mailto:nicholas@csoai.org?subject=Request%20${encodeURIComponent(fw.name)}%20crosswalk%20PDF`}
              >
                <Button
                  size="lg"
                  variant="outline"
                ...
                >
                  <FileText className="h-4 w-4 mr-2" /> Request crosswalk PDF
                </Button>
              </a>
            )}
          </div>
