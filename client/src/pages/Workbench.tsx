...
// The Claude-Science pattern, applied to AI governance: an assistant plus a
// skill/MCP palette. Every artifact reports its actual evidence state. A signer
// response and a local SHA-256 hash are deliberately different states; neither
// implies that Council review is live.
...
async function seal(text: string): Promise<{ kind: string; fp: string; sig: string }> {
  try {
    ...
    if (r.ok) { const d = await r.json(); const sig = String((d && (d.signature || d.sig)) || ""); const fp = String((d && (d.fingerprint || d.publicKey || d.key)) || ""); if (sig && fp) return { kind: "Ed25519 response · verify before relying", fp: fp.slice(0, 40), sig: sig.slice(0, 48) }; }
  } catch (e) {}
  try {
...
export default function Workbench() {
  ...
  async function run() {
    ...
    setBusy(true);
    const res = await askSovereign(question, { system: "You are the CSOAI Council assistant coordinating a governance workbench. Skill in use: " + skill.name + " (" + skill.hint + "). Produce a concise, concrete governance draft — cite the relevant frameworks/obligations. AI governance & cybersecurity only; never a companion. Do not imply Council review or a cryptographic signature unless a verified artifact is attached.", fallback: "The assistant endpoint is unreachable. The workbench can record only a local content hash for this fallback; retry for a reasoned draft." });
  ...
  return (
    <div className="min-h-screen bg-[#03080e] text-emerald-50">
      <div className="mx-auto max-w-6xl px-5 py-10">
        ...
        <h1 className="mt-3 text-3xl sm:text-4xl font-black tracking-tight">Every output — <span className="bg-gradient-to-r from-emerald-300 to-teal-300 bg-clip-text text-transparent">with explicit provenance and evidence state.</span></h1>
        <p className="mt-3 max-w-3xl text-emerald-100/75 text-[15px]">The AI-workbench pattern, applied to AI governance. The assistant prepares an artifact and records whether it received an Ed25519 response, only a local SHA-256 content hash, or no seal. Verify a signature independently before relying on it. Council review is a designed layer (Charter Art. 11), not a live pipeline — measured status on the <a href="/refutation-ledger" className="underline text-emerald-300">Refutation Ledger</a> (DR-0007).</p>
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-[260px_1fr]">
          ...
          <div className="rounded-2xl border border-emerald-500/20 bg-[#04120c] p-3">
            ...
            <a href="/dashboard?tab=home" className="mt-3 block rounded-lg border border-emerald-500/20 px-3 py-2 text-center text-[12px] font-semibold text-emerald-200/80 hover:bg-white/5">Published tools →</a>
          </div>
          ...
          <div>
            <div className="rounded-2xl border border-emerald-500/20 bg-[#04120c] p-4">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={run} disabled={busy} className="rounded-lg bg-emerald-500 px-4 py-2.5 text-sm font-bold text-[#03110b] hover:bg-emerald-400 disabled:opacity-50">{busy ? "Recording…" : "Run + record"}</button>
              </div>
              <div className="mt-2 text-[11px] text-emerald-300/45">Assistant draft · evidence state shown per result · Council review designed, not live (DR-0007)</div>
            </div>

            {arts.length === 0 ? (
              <div className="mt-4 rounded-2xl border border-dashed border-emerald-500/20 p-8 text-center text-sm text-emerald-100/40">Run a skill to produce a governance draft with an explicit evidence state.</div>
            ) : (
                ...
                {arts.map((a) => (
                  <div key={a.id} className="rounded-2xl border border-emerald-500/20 bg-white/[0.02] p-4">
                    <div className="flex flex-wrap items-center gap-2 text-[11px]">
                      ...
                      <span className="ml-auto rounded-full border border-emerald-400/25 px-2 py-0.5 font-mono text-emerald-300/70">Evidence: {a.sealKind}</span>
                    </div>
                    ...
                    <div className="mt-3 grid gap-2 border-t border-emerald-500/15 pt-3 text-[11px] sm:grid-cols-2">
                      ...
                      <div className="text-emerald-200/70 sm:col-span-2">⚖ {a.council} · inspect the evidence state above before relying on this draft</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-8">
          <div className="mb-2 text-[11px] font-bold uppercase tracking-wide text-emerald-300/60">Or inspect a tool endpoint directly</div>
          <ToolRunner />
        </div>

        <p className="mt-8 text-[11px] text-emerald-300/40">Honest note: a verified Ed25519 signer response and an in-browser SHA-256 content hash are different states. A local hash proves only the draft bytes; it is not a signature. Council review is a designed layer (Charter Art. 11), not a live pipeline — measured status on the Refutation Ledger (DR-0007).</p>
      </div>
    </div>
  );
}
