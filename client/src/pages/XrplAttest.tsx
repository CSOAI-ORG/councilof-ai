 ...
 * (writes_board false). Signed count derived at load from /api/xrpl;
...
export default function XrplAttest() {
  ...
  const [xrplSigned, setXrplSigned] = useState<number | null>(null);
  const [xrplTotal, setXrplTotal] = useState<number | null>(null);
  const [err, setErr] = useState<string | null>(null);

  useEffect(() => {
    const derived = (r: unknown) =>
      Array.isArray((r as { assets?: unknown[] }).assets) ? (r as { assets: unknown[] }).assets : [];
    document.title = "XRPL public-root catalogue | Council of AI";
    setMetaDescription(
      "GET /root.json is the living XRPL catalogue. GET /api/xrpl: signed count derived at load (EURQ/USDQ unsigned, NO_LAPTOP_SIGN). writes_board false. DEVNET historical. Not a GSPC grade.",
    );
    fetch("/root.json")
    ...
    fetch("/api/xrpl")
      ...
      .then((r) => {
        const assets = r ? derived(r) : [];
        setXrplTotal(assets.length || null);
        setXrplSigned(
          assets.length
            ? assets.filter((a: { sig_ed25519?: unknown }) => a?.sig_ed25519 != null).length
            : null,
        );
      })
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 via-white to-white">
      <div className="mx-auto max-w-4xl px-6 py-14">
        ...
        <p className="mt-4 max-w-3xl text-gray-600">
          ...
          <strong>{xrplSigned == null ? "loading…" : `${xrplSigned}/${xrplTotal ?? "?"}`} GH-secret sigs</strong>
          (derived at load); <code>EURQ</code>/<code>USDQ</code> unsigned
        ...
        <div className="mt-8 rounded-xl border border-amber-300 bg-amber-50 p-5 text-sm text-amber-900">
          ...
          <p className="mt-2">
            ...
            <code>writes_board</code> is false. Live n={xrplTotal ?? "…"}: {xrplSigned == null ? "…" : `${xrplSigned}/${xrplTotal ?? "?"}`} GH-secret sigs; EURQ/USDQ unsigned.
        ...
        {root && (
          <dl className="mt-8 grid gap-3 sm:grid-cols-2 text-sm">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <dd className="mt-1 font-mono">{xrplSigned == null ? "loading…" : `${xrplSigned}/${xrplTotal ?? "?"}`} GH-secret; EURQ/USDQ unsigned</dd>
            </div>
          </dl>
        )}
        ...
        <p className="mt-8 text-sm text-gray-600">
          ...
          (estate mode). Public-root mode on that page loads this same catalogue: {xrplSigned == null ? "…" : `${xrplSigned}/${xrplTotal ?? "?"}`} GH-secret
