      ...
      <section className="relative py-20 bg-gradient-to-br from-primary/10 via-background to-blue-500/10">
        <div className="container mx-auto px-4">
          <motion.div className="max-w-4xl mx-auto text-center" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <Badge variant="outline" className="mb-4"><FileCheck className="h-3 w-3 mr-1" />Legal Document</Badge>
            <h1 className="text-4xl md:text-5xl font-bold mb-6">AI Safety Licensing Agreement</h1>
            <p className="text-xl text-muted-foreground mb-4">Council Safety of Artificial Intelligence (CSOAI)</p>
          </motion.div>
        </div>
      </section>
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-5xl mx-auto space-y-12">
          <motion.section {...fadeInUp}>
            <Card><CardHeader><CardTitle className="flex items-center gap-2"><Info className="h-5 w-5 text-primary" />Key Definitions</CardTitle></CardHeader>
              <CardContent><div className="grid md:grid-cols-2 gap-4">
                {[{ term: "AI System", definition: "Any artificial intelligence system submitted for CSOAI licensing" },{ term: "License", definition: "Authorization to operate AI systems under CSOAI governance" },{ term: "License Tier", definition: "Classification based on AI system risk level (Articles 6-9 of Charter)" },{ term: "Council", definition: "CSOAI's automated monitoring infrastructure" },{ term: "Charter", definition: "The CSOAI 52-article Partnership Charter" }].map((item, index) => (
                  <div key={index} className="p-3 rounded-lg bg-muted"><p className="font-semibold text-sm">{item.term}</p><p className="text-sm text-muted-foreground">{item.definition}</p></div>
                ))}
              </div></CardContent>
            </Card>
          </motion.section>
          <div className="text-center text-sm text-muted-foreground pt-8 border-t"><p>© 2026 Council Safety of Artificial Intelligence. All Rights Reserved.</p></div>
        </div>
      </div>
    </div>
  );
