      ...
      <div className="section-shell">
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          <div className="text-center lg:text-left">
            ...
            <h1 className="mt-6 text-3xl font-black tracking-tight text-white sm:text-4xl lg:text-5xl">Council OS</h1>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-center lg:justify-start">
              <a href="/os" className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-7 py-3.5 text-sm font-bold text-white shadow-lg shadow-emerald-500/25 transition-all hover:bg-emerald-400 hover:shadow-emerald-400/30">
                Open Council OS <ChevronRight className="h-4 w-4" />
              </a>
              <a href="/gspc-verify" className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/30 bg-transparent px-7 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-white/10">
                Verify a card — free
              </a>
            </div>
          </div>
          <div className="relative mx-auto w-full max-w-xl lg:mx-0">
            <div className="overflow-hidden rounded-2xl shadow-2xl shadow-emerald-900/30">
              <img src="/images/coliseum_hero_arena.jpg" alt="White arena with humans in green and white facing humanoid robots - the Council OS measurement space" className="w-full object-cover" loading="eager" />
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <section id={id} className="surface-raised py-10 sm:py-12">
      <div className="section-shell text-center">
        <a href="/os" className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-8 py-4 text-base font-bold text-white shadow-lg shadow-emerald-600/25 transition-all hover:bg-emerald-500 hover:shadow-emerald-500/30">
          Open Council OS <ChevronRight className="h-5 w-5" />
        </a>
        <p className="mt-4 text-sm text-muted-foreground">Free and loginless. Board, verifier, assessment, evidence — one window.</p>
      </div>
    </section>
  );
}
    ...
    <section className="surface-sunken section-y">
      <div className="section-shell">
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {OUTCOMES.map((o) => (
            <a key={o.title} href={o.href} className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-card transition-shadow hover:shadow-lg">
              {o.image && (
                <div className="relative aspect-[16/9] overflow-hidden">
                  <img src={o.image} alt="" className="absolute inset-0 h-full w-full object-cover transition-transform group-hover:scale-105" loading="lazy" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                  {o.status && (
                    <span className="absolute right-3 top-3 rounded-full bg-amber-500/90 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-white">{o.status}</span>
                  )}
                </div>
              )}
              <div className="flex flex-1 flex-col p-5">
                <div className="flex items-start gap-3">
                  ...
                  <h3 className="text-base font-bold leading-snug text-foreground group-hover:text-primary">{o.title}</h3>
                </div>
  ...
  return (
    <section className="section-y-sm surface-ink border-y" style={{ borderColor: "var(--ink-border)" }}>
      <div className="section-shell">
        ...
        <p className="ink-muted mt-5 text-center text-[11px]">We are not certified to SOC 2 or ISO 42001. Measurement credential, never certification.</p>
      </div>
    </section>
  );
}
    ...
    <section className="surface-sunken py-12 sm:py-16">
      <div className="section-shell">
        <div className="mx-auto max-w-2xl rounded-2xl border border-emerald-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Paste a signed measurement record. Your browser recomputes the hash and checks the Ed25519 signature. Nothing is sent to us. No account, no fee, permanently.</p>
          <a href="/gspc-verify" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-6 py-3 text-sm font-bold text-white transition hover:bg-emerald-700">
    ...
    <main className="surface-base">
      ...
      <section className="surface-raised py-8">
        <div className="section-shell flex flex-wrap items-center justify-center gap-4 text-sm">
          <a href="/faq" className="inline-flex items-center gap-2 font-bold text-primary hover:underline">Questions? See all FAQs <ChevronRight className="h-4 w-4" /></a>
          <span className="text-muted-foreground">·</span>
          <a href="/blog" className="inline-flex items-center gap-2 font-bold text-primary hover:underline">News <ChevronRight className="h-4 w-4" /></a>
        </div>
      </section>
    </main>
  );
