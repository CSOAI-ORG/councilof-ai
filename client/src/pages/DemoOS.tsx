...
const STEPS: Step[] = [
  ...
  { say: "Now up to orbit - the instrument on a real-world globe. Measurement slots on the living board. Empty cells stay empty. Each measured slot is anchored where its law was made.", wins: [{ title: "◉ Council Space — the measurement globe, live", src: "/gspc-arena", slot: "tr" }], fly: { lng: -0.118, lat: 40, height: 22000000 }, layer: { tag: "sats", on: true } },
  ...
  { say: "And the signed event world - J-Space. Every event the estate has ever signed, embedded in hyperbolic space. Zoom forever.", wins: [{ title: "◉ J-Space — signed events", src: "/j-space", slot: "tr" }], fly: { lng: -79.38, lat: 43.65, height: 16000 }, full: true },
  ...
  { say: "And it's not a dashboard - it's a whole operating system. Governed tools, keyless and live, from crosswalks to cyber to attestation, all running on one Council engine. An app store for AI governance. No one else ships it this way.", wins: [{ title: "The Council OS — workbench", src: "/workbench", slot: "c" }], full: true },
  ...
  { say: "And this is our own Council network - signed agents, from proofof.ai to safetyof.ai, each one accountable and each one arced back to a single council. This is the ecosystem, live: one crown, many agents, all sealed to Layer 0.", layer: { tag: "network", on: true }, fly: { lng: 2, lat: 52, height: 4200000 } },
  { say: "Here's why this changes everything. The barriers that stop most teams - hiring consultants, months of manual mapping, tools that don't talk to each other - I remove them. The rail is free. Verification is free forever. Where evidence is sold it is a signed artefact, never a grade.", wins: [{ title: "Get measured", src: "/assess", slot: "tr" }], full: true },
  ...
  { say: "Now the designed council visualisation. Thirty-three designed seats, spiralling into a vote. This is a design figure - the live consensus guarantee was retracted (DR-0007). What we ship is measurement, signed, including the failures.", cmd: { cmd: "bftSpiral" }, full: true },
  ...
  { say: "Here's the model registry - every published axis, every leader named, every score recomputable from the published harness. Slot counts stay on GET /api/gspc.", wins: [{ title: "▦ Model Registry — live", src: "/models", slot: "tr" }], fly: { lng: -0.118, lat: 51.509, height: 60000 } },
  ...
  { say: "Watch - the fleet measurement board updates as new runs land. Every published number carries its sample size and interval. Empty cells stay empty.", layer: { tag: "threat", on: true }, rearm: true, fly: { lng: -0.1, lat: 51.5, height: 1400000 } },
  ...
  { say: "Own your AI. Own your data. The rail is free. Verify forever. That's your OS - and I'm always right here. Ask me anything, any time.", wins: [{ title: "Get measured", src: "/assess", slot: "tr" }], home: true },
];
        ...
        <div className="absolute inset-0 z-[60]" onClick={() => setDrawer(false)}>
          ...
          <div onClick={(e) => e.stopPropagation()} className="absolute right-0 top-0 h-screen w-[360px] max-w-[92vw] overflow-y-auto bg-white text-gray-900 shadow-2xl">
            ...
            <div className="p-4">
              ...
              <a href="/?lobby=home" className="mt-3 block rounded-xl bg-emerald-600 px-3 py-2.5 text-center text-sm font-bold text-white hover:bg-emerald-500">Enter Council OS →</a>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="border-t border-emerald-500/15 p-3">
            {ending ? (
              <div className="grid grid-cols-2 gap-2">
                ...
                <a href="/?lobby=home" className="rounded-xl bg-emerald-500/15 px-3 py-2 text-center text-xs font-bold text-emerald-100 hover:bg-emerald-500/25">Show governance</a>
                <a href="/?lobby=home" className="rounded-xl bg-emerald-500 px-3 py-2 text-center text-xs font-bold text-[#03110b] hover:bg-emerald-400">Enter the OS ▶</a>
              </div>
            ) : !paused ? (
