...
export function InsurersDataUnavailable({
  source,
}: {
  source: "board" | "reported";
}) {
  const isBoard = source === "board";
  const endpoint = isBoard ? "/api/gspc" : "/api/reported";

  return (
    <p
      className="mt-6 rounded-xl border border-gray-300 bg-white p-4 text-sm text-gray-700"
      data-testid={`${source}-unavailable`}
      role="status"
    >
      <strong>
        {isBoard ? "The live board" : "Third-party context"} is unavailable in
        this rendered view.
      </strong>{" "}
      {isBoard
        ? "No measurement values have been substituted."
        : "No reported figures have been substituted."}{" "}
      Check the source endpoint directly at{" "}
      <a href={endpoint} className="font-semibold text-emerald-700 underline">
...
function InsurersEvidencePack() {
  ...
  useEffect(() => {
    ...
    setMetaDescription(
      "Evidence an underwriter can verify: Ed25519-signed AI measurement cards, recomputable from published rows. Council of AI (CSOAI LTD, UK 16939677) — measurement, not certification. Live board: GET /api/gspc.",
    );
    ...
    fetchWithRetry("/api/reported")
      ...
      .catch(() => setReportedUnavailable(true));
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 via-white to-white">
      ...
      <div className="mx-auto max-w-5xl px-6 py-14">
        ...
        <p className="mt-4 max-w-3xl text-lg text-gray-600">
          We measure AI systems against the rules that govern them, sign the
          result, and publish what we cannot measure. Every number on this page
          is either fetched live from a signed endpoint or labelled with its
          third-party source and capture date — nothing is blended.
        </p>
        <p className="mt-3 max-w-3xl text-sm text-gray-600">
          What this is <strong>not</strong>: not a certification, not a
          conformity mark, not a legal determination — it is a measurement
          record you can recompute yourself.
        </p>
        ...
        <div className="mt-8 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-xl border border-emerald-600/20 bg-white p-4">
            ...
            <p className="mt-1 text-gray-600">
              Our own signed, deterministic runs. Live at{" "}
              <code>GET /api/gspc</code>.
            </p>
          </div>
          <div className="rounded-xl border border-gray-300 bg-white p-4">
            <p className="font-bold text-gray-600">UNMEASURED</p>
            <p className="mt-1 text-gray-600">
              Honestly withheld, with the reason stated — insufficient n, or no
              separation test yet. Empty cells stay empty.
            </p>
          </div>
          <div className="rounded-xl border border-amber-300 bg-white p-4">
            <p className="font-bold text-amber-700">REPORTED</p>
            <p className="mt-1 text-gray-600">
              Third-party figures, cited and dated — reported by the source, not
              measured here. Live at <code>GET /api/reported</code>.
            </p>
          </div>
        </div>
        ...
        <p className="mt-2 max-w-3xl text-gray-600">
          A measurement card is a small (~3KB) signed record. Each part exists
          so an actuary can check it without trusting the issuer:
        </p>
        <ul className="mt-4 space-y-3">
          {CARD_ANATOMY.map((item) => (
            <li
              key={item.term}
              className="rounded-xl border border-emerald-600/15 bg-white p-4"
            >
        ...
        <p className="mt-2 max-w-3xl text-gray-600">
          A stranger with a terminal can check us. No account, no key exchange,
          no permission:
        </p>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-emerald-600/15 bg-white p-5">
            ...
            <p className="mt-1 text-sm text-gray-600">
              The board shows which axis a system fails, with the n behind each
              result. A failure rate with a sample size and a Wilson interval is
              a frequency input, not a marketing claim.
            </p>
          </div>
          <div className="rounded-xl border border-emerald-600/15 bg-white p-5">
            <p className="font-bold text-gray-900">Severity tails</p>
            <p className="mt-1 text-sm text-gray-600">
              Where n≥100, the board publishes <code>mean_harm</code> and{" "}
              <code>cvar05_harm</code> per axis. CVaR@5% is the average harm
              across the worst 5% of items — the tail an underwriter prices, not
              the average day. Where n&lt;100 the field is honestly null.
            </p>
          </div>
          <div className="rounded-xl border border-emerald-600/15 bg-white p-5">
            <p className="font-bold text-gray-900">Drift</p>
            <p className="mt-1 text-sm text-gray-600">
              Regulation changes; measurements go stale. A daily reg-watch
              detector watches the governing corpus, and state changes are
              published to{" "}
              <a
                href="/api/feed.xml"
                className="font-semibold text-emerald-700 underline"
          ...
          <div className="rounded-xl border border-emerald-600/15 bg-white p-5">
            ...
            <p className="mt-1 text-sm text-gray-600">
              ...
              we publish our own models losing our own arena. An instrument that
              catches its owner is the one an underwriter can verify; we do not
              sell a rating, and the underwriter still prices.
            </p>
          </div>
        </div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-gray-600">
          Fetched live from <code>GET /api/gspc</code> (the live count lives
          there, not here). A <strong>TIE</strong> means the leader&apos;s edge
          is statistically indistinguishable — ties are never counted as wins.
        </p>

        {boardUnavailable && <InsurersDataUnavailable source="board" />}
        {!board && !boardUnavailable && (
          <p className="mt-6 text-gray-600">Loading the live board…</p>
        )}

        {board && board.axes.length === 0 && (
          <p className="mt-6 rounded-xl border border-gray-300 bg-white p-4 text-sm text-gray-700">
            The board returned no axes. That is the endpoint&apos;s answer, not
            a rendering failure — check <code>GET /api/gspc</code> directly.
          </p>
        )}
                  ...
                  return (
                    <tr key={a.axis} className="border-b last:border-0">
                      ...
                      >
                        ...
                        {acc.state === "facts" && (
                          <span title={acc.title} className="text-gray-600">
                            {acc.text}
                            {acc.detail && (
                              <span className="ml-1 block font-sans text-[11px] text-gray-600">
                                {acc.detail}
                              </span>
                            )}
                          </span>
                      ...
                      <td className="p-3">
                        {a.separation ? (
                          <span
                            className={`inline-block whitespace-nowrap rounded-full border px-2.5 py-0.5 text-xs font-bold ${CHIP[a.separation]}`}
                          >
                            {a.separation === "TIE"
                          ...
                          <StatusChip
                            kind={chipFor(a.status, a.separation, a.kind)}
                          />
                        )}
                        {sepNote && (
        ...
        <p className="mt-3 text-xs text-gray-600">
          Full per-axis detail — Wilson intervals, fleet means, harm tails, the
          signature:{" "}
          <Link
        ...
        <p className="mt-2 max-w-3xl text-sm text-gray-600">
          Figures published by others, cited with their capture date. Each entry
          is <strong>reported by the source, not measured here</strong> —
          unsigned, and never enters the board.
        </p>

        {/* The neutral unavailable state deliberately avoids the prerender guard's failure
            sentinel while still stating that no figures were substituted. REPORTED is
            supplementary; no board measurement depends on it. */}
        {reportedUnavailable && <InsurersDataUnavailable source="reported" />}
        {!reported && !reportedUnavailable && (
        ...
        {reported && reported.entries.length === 0 && (
          <p className="mt-6 rounded-xl border border-amber-300/60 bg-amber-50/40 p-4 text-sm text-gray-700">
            No third-party figures are currently carried. An empty REPORTED set
        ...
        <p className="mt-10 text-xs text-gray-600">
          Measurement, not certification. CSOAI Ltd · UK Companies House
          16939677 ·{" "}
          <a href="mailto:nicholas@csoai.org" className="underline">
            nicholas@csoai.org
          </a>
          . Every MEASURED number on this page is recomputable from{" "}
          <code>GET /api/gspc</code>; every REPORTED figure carries its source
          and capture date; what we cannot measure is withheld and says so.
        </p>
      </div>
    </div>
  );
