 ...
 *
 * All entries are ours. Numbers 7 and 8 are the ones that hurt.
 * Index-claim rows (kind: index-claim) are honesty doctrine — not architecture bets.
 */
...
const LEDGER = [
  ...
  },
  {
    n: 10,
    kind: "index-claim",
    claim: "Empty labour / AI-economy indices mean the product is incomplete — fill them overnight with GPU scores",
    measured: "UNMEASURED · measured_score: null · fused_into_gspc: false (GET /api/indices)",
    artefact: "docs/SOVOS/INDEX-METHOD-0.1.md · /indices · .claude/skills/refuse-measured-labour",
    why: "Refuted by doctrine: inventing MEASURED labour or AI-economy scores is worse than empty cells. Indices are a contextual firewall — never GSPC grading inputs. RunPod does not authorize filling them. Empty is honesty until a bank freezes.",
  },
];
    ...
    <div className="min-h-screen bg-[#03110b] text-emerald-50">
      ...
      <section className="mx-auto max-w-4xl px-6 py-12 space-y-4">
        ...
        <div className="mt-8 rounded-2xl border border-emerald-500/20 bg-[#05140d] p-6 text-[13px] text-emerald-100/80 leading-relaxed">
          <p>
            <strong className="text-emerald-50">What the pattern says.</strong> Six of the first
            eight architectural bets were attempts to make a small model behave like a larger one —
            by routing between copies of it, feeding it statute, voting across prompts of it,
            changing its decoding. All six failed. Capability comes from the base model; the
            wrapper makes it cheaper, grounded and auditable, <em>not smarter</em>. We say so
            because we spent six experiments proving it. Entry #10 is a separate index-claim
            refutation: empty labour/economy indices stay UNMEASURED — GPU does not fill them.
          </p>
          <p className="mt-3">
