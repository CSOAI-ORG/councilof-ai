...
const navigation: NavGroup[] = [
  ...
  },
  {
    ...
    submenu: [
      ...
      { name: 'Get measured', href: '/os?lobby=assess', description: 'Start a signed assessment of your own system' },
      ...
      { name: 'Readiness assessment', href: '/os?lobby=assess', description: 'Work out which duties bind you before you measure' },
      ...
      <nav id="navigation" className="container mx-auto px-4 sm:px-6 lg:px-8" aria-label="Main navigation">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center" ref={dropdownRef}>
            <div className="flex items-center gap-1 2xl:gap-3">
              ...
              {PRIMARY_LINKS.map((item) => (
                <a key={item.href} href={item.href} className={`px-3 py-2 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${isActive(item.href) ? 'text-emerald-700 bg-emerald-50' : 'text-muted-foreground hover:text-emerald-700 hover:bg-muted'}`}>
                  {item.name}
                </a>
              ))}
              </div>
              ...
              {navigation.map((item) => (
                <div key={item.name} className="relative" onMouseEnter={() => handleMouseEnter(item.name)} onMouseLeave={handleMouseLeave}>
                  <button data-nav-trigger={item.name} aria-haspopup="true" aria-expanded={activeDropdown === item.name} aria-controls={`nav-panel-${item.name.replace(/\s+/g, '-').toLowerCase()}`} className={`px-2 2xl:px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-1 whitespace-nowrap ${isActive(item.href) || activeDropdown === item.name ? 'text-emerald-700 bg-emerald-50' : 'text-muted-foreground hover:text-emerald-700 hover:bg-muted'}`} onClick={() => setActiveDropdown(activeDropdown === item.name ? null : item.name)} onFocus={() => handleMouseEnter(item.name)}>
                    {item.name}
                    <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${activeDropdown === item.name ? 'rotate-180' : ''}`} />
                  </button>
                  {activeDropdown === item.name && (
                    <div id={`nav-panel-${item.name.replace(/\s+/g, '-').toLowerCase()}`} className="absolute left-0 top-full pt-2 z-50" onMouseEnter={() => handleMouseEnter(item.name)} onMouseLeave={handleMouseLeave}>
                      <div className="w-72 max-h-[min(72vh,40rem)] overflow-y-auto rounded-xl border border-border bg-popover shadow-xl">
                        ...
                        <div className="py-2">
                          {item.submenu.map((subItem) => (
                            <div key={subItem.href + subItem.name}>
                              {subItem.section && (
                                <div className="px-4 pt-3 pb-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{subItem.section}</div>
                              )}
                              <a href={subItem.href} target={subItem.external ? '_blank' : undefined} rel={subItem.external ? 'noreferrer' : undefined} className="block px-4 py-2.5 hover:bg-muted focus:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-500 transition-colors group" onClick={() => setActiveDropdown(null)}>
                                <div className="font-medium text-foreground group-hover:text-emerald-700 text-sm">
                                  {subItem.name}
                                  {subItem.external && (<span className="ml-1.5 align-middle text-[9px] font-bold uppercase tracking-wide text-muted-foreground">JSON</span>)}
                                </div>
                        ...
                        <div className="px-4 py-2 bg-muted border-t border-border">
                          <a href={item.href} className="text-xs text-emerald-600 hover:text-emerald-700 font-medium" onClick={() => setActiveDropdown(null)}>View all {item.name.toLowerCase()} &rarr;</a>
                        </div>
                      </div>
                    </div>
                  )}
          ...
          <div className="hidden md:flex flex-nowrap items-center gap-2 2xl:gap-3">
            <a href="/os" className="hidden xl:inline-flex rounded-lg bg-emerald-700 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-800">Council OS</a>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden py-4 border-t border-border max-h-[calc(100vh-4rem)] overflow-y-auto">
            <div className="space-y-1">
              <a href="/" className={`block px-4 py-3 rounded-lg font-medium ${location === '/' ? 'text-emerald-700 bg-emerald-50' : 'text-foreground/80'}`} onClick={() => setMobileMenuOpen(false)}>Home</a>
              {PRIMARY_LINKS.map((item) => (
                <a key={item.href} href={item.href} className={`block px-4 py-3 rounded-lg font-medium ${isActive(item.href) ? "text-emerald-700 bg-emerald-50" : "text-foreground/80"}`} onClick={() => setMobileMenuOpen(false)}>{item.name}</a>
              ))}
              ...
              {navigation.map((item) => (
                <div key={item.name} className="space-y-1">
                  <a href={item.href} className={`flex items-center gap-3 px-4 py-3 rounded-lg font-medium ${isActive(item.href) ? 'text-emerald-700 bg-emerald-50' : 'text-foreground/80'}`} onClick={() => setMobileMenuOpen(false)}>
                    <item.icon className="h-5 w-5 text-emerald-600" />{item.name}
                  </a>
                  <div className="ml-12 space-y-1">
                    {item.submenu.map((subItem) => (
                      <div key={subItem.href + subItem.name}>
                        {subItem.section && (<div className="px-4 pt-3 pb-0.5 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{subItem.section}</div>)}
                        <a href={subItem.href} target={subItem.external ? '_blank' : undefined} rel={subItem.external ? 'noreferrer' : undefined} className="block px-4 py-2 text-sm text-muted-foreground hover:text-emerald-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 rounded" onClick={() => setMobileMenuOpen(false)}>
                          {subItem.name}{subItem.external && <span className="ml-1 text-[10px] uppercase tracking-wide text-muted-foreground">JSON</span>}
                        </a>
                      </div>
                    ))}
                  </div>
