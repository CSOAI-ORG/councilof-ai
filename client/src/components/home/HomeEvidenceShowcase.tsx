...
import { buildBlogIndex, UNLISTED_UNTIL_REWRITTEN } from "@/lib/blogIndex";
...
const FEATURED_SLUGS = [
  ...
  "why-we-publish-our-refutations",
] as const;
...
export function featuredEvidencePosts() {
  const bySlug = new Map(
    buildBlogIndex()
      .filter((post) => !UNLISTED_UNTIL_REWRITTEN.has(post.slug))
          ...
          {posts.map((post, index) => (
            ...
            >
              ...
              <div className="mt-auto flex items-center justify-between gap-3 pt-5 text-xs font-semibold text-slate-500">
                <span>{post.dateLabel ?? "Date unmeasured"}</span>
