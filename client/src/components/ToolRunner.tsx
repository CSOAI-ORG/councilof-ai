...
export function initialToolDraft(tool: SovTool): ToolDraft {
...
export function isPaidTool(tool: SovTool): boolean {
  return tool.csoai?.paid === true;
}

export function resultOutcome(result: ToolResult): string | null {
...
export default function ToolRunner({
  ...
  useEffect(() => {
    ...
    listTools().then((reply) => {
      ...
      setTools(reply.tools);
      const first =
        reply.tools.find((tool) => tool.name === initialToolName) ||
        reply.tools.find((tool) => tool.name === "board_totals") ||
        reply.tools[0] ||
        null;
      setActive(first);
  ...
  async function run() {
    ...
    setOutput(null);
    setBusy(true);
    const result = await callTool(active.name, parsed.args);
