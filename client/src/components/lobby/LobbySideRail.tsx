...
export default function LobbySideRail({
  ...
  const drive = (chip: DriveChip) => {
    // Chat-first: seed Ask only. Do not navigate the site column here —
    // path hops (e.g. /gspc-scoreboard) remount the page and drop the dock.
    // Pane + prompt stay in the lobby; open the site path from the centre
    // "Open … →" control when the reader wants the page beside the dock.
    openLobby({
      pane: chip.pane ?? "home",
      task: chip.task,
      prompt: chip.prompt,
