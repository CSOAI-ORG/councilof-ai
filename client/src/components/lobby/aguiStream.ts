...
export type AguiStreamHandlers = {
  ...
  onToolResult?: (payload: AguiToolResult) => void;
...
async function consumeSseBody(
  ...
  while (true) {
    ...
    for (const frame of frames) {
      ...
      if (event === "TOOL_CALL_START" || event === "TOOL_CALL_ARGS") {
        const n = d.toolCallName ?? d.name ?? d.tool;
        if (typeof n === "string") pendingToolName = n;
      }
      if (event === "TOOL_CALL_RESULT" && d.result) {
        const live = d.result as Record<string, unknown>;
        const name =
          (typeof d.toolCallName === "string" && d.toolCallName) ||
          (typeof d.name === "string" && d.name) ||
          pendingToolName;
        handlers.onToolResult?.({ name: name || null, result: live });
