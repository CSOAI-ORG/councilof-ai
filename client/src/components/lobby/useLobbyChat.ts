 ...
 *
 * Measured MCP tools render as controlled cards (tool → React card registry).
 */
    ...
    async (raw: string, onNavigate: (t: LobbyTab) => void, opts?: { aguiHandle?: string }) => {
          ...
          (
            {
              board: "gspc_board",
              models: "gspc_board",
              "east-west": "east_west_board",
              verify: "verify_tally",
              ecosystem: "ecosystem_index",
              routes: "instruments_catalog",
              tools: "instruments_catalog",
            } as Record<string, string>
          )[tab.id] ?? null;
        const toolCards = paneTool ? [cardForTool(paneTool, { phase: "done" })!].filter(Boolean) : undefined;
        push({
          role: "council",
          text:
      ...
      try {
        ...
        if (wireUp) {
          ...
          const collectedCards: ToolCardInstance[] = [];
          const agui = await runAguiSession(
            question,
            {
              ...
              onToolResult: ({ name, result }) => {
                const tool = name || inferToolFromResult(result);
                if (!tool) return;
                ...
                setThreads((prev) =>
                  prev.map((x) => {
                    ...
                    const i = turns.length - 1;
                    if (i < 0 || turns[i].role !== "council") return x;
                    const existing = turns[i].toolCards ?? [];
                    turns[i] = {
                      ...turns[i],
                      toolCards: [...existing.filter((c) => c.tool !== card.tool), card],
                    };
                    return { ...x, turns };
                  }),
                );
          ...
          if (agui?.text) {
            updateLastCouncilTurn(threadId, {
              ...
              toolCards: collectedCards.length ? collectedCards : undefined,
            });
            return;
          }
