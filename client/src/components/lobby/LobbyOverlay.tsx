...
import { DEFAULT_TAB, LOBBY_TABS, isOsRailTab, isSiteDoor, paneLoadFor, workspacePaneLoadFor, softwareLeavesOs, SOFTWARE_HREF, tabById, type LobbyTab, type LobbyTabId } from "./tabs";
...
import { LOBBY_TASKS, OS_ASK_EVENT, type LobbyIntent } from "@/lib/lobbyLink";
...
export default function LobbyOverlay({
  ...
  /** overlay = floating dialog. page = /os product workspace (not OsShell). */
  mode?: "overlay" | "page";
  /** Sync the host URL when a rail tab is chosen on /os. */
  onHostTab?: (t: LobbyTab) => void;
}) {
  ...
  const [tabId, setTabId] = useState<LobbyTabId>(() => {
    ...
    const t = tabById(id);
    if (!isOsRailTab(id)) return DEFAULT_TAB;
    if (t.path && isUnframeable(t.path)) return DEFAULT_TAB;
    return id;
  });
  const [leftOpen, setLeftOpen] = useState(() => readOpen(LEFT_KEY, LEFT_DEFAULT));
  const [rightOpen, setRightOpen] = useState(() => page ? true : readOpen(RIGHT_KEY, RIGHT_DEFAULT));
  // The composer is opened on demand, and stays open once a conversation exists.
  // /os page mode keeps it open so the AI rail and #os-chat exist on first paint.
  const [composerOpen, setComposerOpen] = useState(page);
  ...
  // Esc closes the overlay (not the /os page). Cmd/Ctrl + . minimises the overlay.
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (!page && e.key === "Escape") { onClose(); return; }
      if (!page && e.key === "." && (e.metaKey || e.ctrlKey)) { e.preventDefault(); setMinimised((m) => !m); return; }
  ...
  useEffect(() => {
    const onAsk = () => {
      ...
      setTimeout(() => document.getElementById("os-chat")?.focus(), 0);
    };
  ...
  const loadPane = useCallback((path: string) => {
    ...
    const load = page ? workspacePaneLoadFor(path) : paneLoadFor(path);
