    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white/95 backdrop-blur-md">
      <nav className="mx-auto flex h-12 max-w-5xl items-center justify-between px-4">
        <div className="flex items-center gap-6">
          ...
          <nav aria-label="Council OS sections" className="hidden items-center gap-1 sm:flex">
            {DOORS.map((door) => {
              const active = currentDoor === door.id;
              return (
                <button key={door.id} onClick={() => navigateToDoor(door.id)}
        ...
        <div className="flex items-center gap-2">
          <Link href="/" aria-label="Exit Council OS"
          ...
          <a href="/os?lobby=play" className={`hidden rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 md:inline-flex ${FOCUS}`}>Play</a>
          <a href="/compliance-training-world/catalog.html" className={`hidden rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 lg:inline-flex ${FOCUS}`} title="Industry quests — training attestation, never certification">Train</a>
          <a href="/gspc-verify" className={`hidden rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 md:inline-flex ${FOCUS}`}>Verify free</a>
          {user ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full bg-emerald-50 text-emerald-700 hover:bg-emerald-100"><User className="h-4 w-4" /></Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild><a href="/dashboard" className="flex items-center"><BarChart3 className="mr-2 h-4 w-4" />Dashboard</a></DropdownMenuItem>
                <DropdownMenuItem asChild><a href="/my-courses" className="flex items-center"><BookOpen className="mr-2 h-4 w-4" />My Courses</a></DropdownMenuItem>
                <DropdownMenuItem asChild><a href="/certificates" className="flex items-center"><Award className="mr-2 h-4 w-4" />Certificates</a></DropdownMenuItem>
                <DropdownMenuItem asChild><a href="/settings" className="flex items-center"><Settings className="mr-2 h-4 w-4" />Settings</a></DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={logout} className="text-red-600"><LogOut className="mr-2 h-4 w-4" />Sign Out</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          ) : (
            <Link href="/login"><Button variant="ghost" size="sm" className="text-sm font-medium text-slate-600 hover:text-slate-900">Sign in</Button></Link>
          )}
        </div>
      </nav>
      <nav aria-label="Council OS sections (mobile)" className="flex items-center gap-1 overflow-x-auto border-t border-slate-100 px-4 py-2 sm:hidden">
        {DOORS.map((door) => {
          const active = currentDoor === door.id;
          return (
            <button key={door.id} onClick={() => navigateToDoor(door.id)}
