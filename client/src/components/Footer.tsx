          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3 text-white">
              ...
              <div>
                ...
                <p className="text-emerald-100 text-sm">{POSITIONING.tagline} Live counts at GET /api/gspc — verify any card free.</p>
              </div>
            </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="lg:col-span-1">
            <Link href="/" className="flex items-center space-x-3 mb-4 hover:opacity-80 transition-opacity">
              <svg viewBox="0 0 100 100" className="h-10 w-10" aria-hidden="true">
                ...
                <path d="M50 5 L90 20 L90 50 C90 75 50 95 50 95 C50 95 10 75 10 50 L10 20 Z" fill="url(#footerShieldGradient)" />
              </svg>
              <span className="text-2xl font-bold">CSOAI</span>
            </Link>
            <p className="text-gray-600 text-sm mb-2">{POSITIONING.tagline}</p>
            <p className="text-gray-600 text-sm mb-4 font-medium">{POSITIONING.firewall}</p>
            <div className="flex space-x-4">
              {socialLinks.map((social) => (
                <a key={social.name} href={social.href} target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-green-600 transition-colors" aria-label={social.name}>
            ...
            <div key={section.title}>
              ...
              <ul className="space-y-3">
                {section.links.map((link) => (
                  <li key={link.name}>
                    {link.external ? (
                      <a href={link.href} target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-green-600 text-sm transition-colors">{link.name}</a>
                    ) : (
                      <Link href={link.href} className="text-gray-600 hover:text-green-600 text-sm transition-colors">{link.name}</Link>
                    )}
                  </li>
                ))}
              </ul>
        ...
        <div className="border-t border-gray-200 pt-6 mb-6">
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-sm">
            <Link href="/library" className="text-xs uppercase tracking-wider text-gray-500 hover:text-emerald-700">Library — full archive</Link>
            {SECTORS.map((sector) => (
              <Link key={sector.id} href={`/library/${sector.id}`} className="text-gray-600 hover:text-emerald-700 transition-colors">{sector.title}</Link>
            ))}
          </div>
        </div>
        <div className="border-t border-gray-200 pt-6 mb-8">
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-sm">
            ...
            {frameworkLinks.map((link) => (
              <Link key={link.href} href={link.href} className="text-gray-600 hover:text-emerald-700 transition-colors">{link.name}</Link>
            ))}
          </div>
        </div>
        <div className="border-t border-gray-200 pt-8 flex flex-col md:flex-row justify-between items-center">
          <p className="text-gray-600 text-sm mb-4 md:mb-0">© {currentYear} CSOAI. All rights reserved.</p>
          <div className="flex flex-wrap justify-center gap-6">
            <Link href="/privacy-policy" className="text-gray-600 hover:text-green-600 text-sm transition-colors">Privacy Policy</Link>
            <Link href="/terms-of-service" className="text-gray-600 hover:text-green-600 text-sm transition-colors">Terms of Service</Link>
            <Link href="/membership-agreement" className="text-gray-600 hover:text-green-600 text-sm transition-colors">Membership</Link>
            <Link href="/licensing-agreement" className="text-gray-600 hover:text-green-600 text-sm transition-colors">Licensing</Link>
          </div>
        </div>
        <FooterVerifyStrip />
        <div className="border-t border-gray-200 mt-8 pt-8">
          <p className="text-gray-600 text-xs text-center max-w-4xl mx-auto mb-2">
            This site uses AI systems, including the Council assistant. Every AI surface is disclosed at first interaction under EU AI Act Article 50 and classified publicly on <Link href="/ai-transparency" className="text-emerald-700 underline">/ai-transparency</Link>.
          </p>
        </div>
        <div className="border-t border-gray-200 mt-8 pt-8">
          <p className="text-gray-600 text-xs text-center max-w-4xl mx-auto mb-2">
          ...
          <p className="text-gray-600 text-xs text-center">
            Council of AI — CSOAI Ltd, UK Companies House 16939677, London. Contact: nicholas@csoai.org.
          </p>
        </div>
      </div>
    </footer>
