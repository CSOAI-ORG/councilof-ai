...
export interface LiveMeasurement {
  ...
  registryAxes: number | null;    // from /api/axis-register -> gspc_registry_axes
  axesServed: number | null;      // from /api/axis-register -> axes.length
...
export function useLiveMeasurement() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [cardsRes, axisRes] = await Promise.all([
        ...
        if (!cards || !axis) {
          ...
          setData({
            ...
            registryAxes: axis?.gspc_registry_axes ?? null,
            axesServed: axis?.axes?.length ?? null,
