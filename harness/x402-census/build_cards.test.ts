...
const CARDS = resolve(ROOT, "public/interop/x402-census-cards");
...
describe("x402 settlement cards", () => {
  it("exist, so this suite cannot pass vacuously", () => {
    expect(files.length, "104 settlements carry a tx the host reported").toBe(104);
  });

  it("carry the facts the brief names: host, tx, amount, verdict, latency, version, payTo", () => {
    const need = ["host", "settle_tx", "asked_units", "status", "latency_s", "x402_version", "pay_to"];
    const bad: string[] = [];
    for (const f of files) {
      const c = JSON.parse(readFileSync(resolve(CARDS, f), "utf8"));
      for (const k of need) if (!(k in c.payload)) bad.push(`${f}: payload missing ${k}`);
    }
  ...
  it("are UNMEASURED at n=1 and say what one purchase cannot show", () => {
    ...
    for (const f of files) {
      ...
      // the brief's exact sentence, verbatim: a card must say what n=1 means in words a reader keeps
      if (!c.unmeasured.includes("one purchase at one moment")) bad.push(`${f}: missing the sentence`);
