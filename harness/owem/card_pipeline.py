...
def sign_card(key, card):
    ...
    if HAVE_CRYPTO:
        ...
        card["signature"] = {"alg": "Ed25519", "kid": key["kid"], "public_key": key["public_key"],
                             "sig": "0x" + sig.hex(), "content_id": content_id,
...
def claimguard_verify(card):
    """ClaimGuard claim-vs-signed-artifact: recompute canonical (signature excluded) + Ed25519 verify."""
    ...
    if sig.get("alg") == "Ed25519" and HAVE_CRYPTO:
        ...
        try:
            ...
            return {"verdict": "VALID", "reason": "Ed25519 signature verifies over canonical body",
