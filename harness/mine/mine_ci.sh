...
# Monorepo: default to harness dir when Mac volume absent
if [[ ! -f "$MINE/test_mine.py" && -f "$SCRIPT_DIR/test_mine.py" ]]; then
  MINE="$SCRIPT_DIR"
fi
# Repo smoke (no Mac volume required)
if [[ "${MINE_CI_REPO_ONLY:-0}" == "1" || ! -f "$MINE/mine-learnt.json" ]]; then
  if ! python3 "$SCRIPT_DIR/test_repo_assets.py"; then FAIL=1; fi
  if [ $FAIL -eq 0 ]; then echo "MINE CI: PASS (repo assets)"; exit 0; fi
  echo "MINE CI: FAIL (repo assets)"; exit 1
fi
# 1) tests
