...
if (gspc.status !== 200) fail(`/api/gspc HTTP ${gspc.status}`);
  ...
  try {
    ...
    if (!j.totals || typeof j.totals !== "object")
      fail("/api/gspc missing totals");
    else
      pass(
        `GET /api/gspc — totals present (${j.totals.public_count || "no public_count"})`,
      );
    if (typeof j.totals.axes !== "number") fail("totals.axes missing");
    else pass(`axes live count ${j.totals.axes} (not pinned)`);
    if (typeof j.totals.measured_axes !== "number")
      fail("totals.measured_axes missing");
    else
      pass(`measured_axes live count ${j.totals.measured_axes} (not pinned)`);
  } catch {
    fail("/api/gspc invalid JSON");
  }
}

const board = await get("/gspc-scoreboard");
if (board.status !== 200 || board.body.length < 50000)
  fail(`/gspc-scoreboard thin or ${board.status}`);
...
console.log("\n## AG-UI → Council OS\n");
const agui = await fetch(HOST + "/ag-ui", {
  redirect: "manual",
  headers: { "user-agent": UA },
});
const agLoc = agui.headers.get("location") || "";
if (agui.status === 308 && agLoc.includes("/dashboard?tab=home"))
  pass("/ag-ui 308 → dashboard home");
else
  fail(`/ag-ui must 308 to /dashboard?tab=home (got ${agui.status} ${agLoc})`);

const aguiAlias = await fetch(HOST + "/agui", {
  redirect: "manual",
  headers: { "user-agent": UA },
});
const loc = aguiAlias.headers.get("location") || "";
if (aguiAlias.status === 308 && loc.includes("/dashboard?tab=home"))
  pass("/agui 308 → dashboard home");
else
  fail(
    `/agui must 308 to /dashboard?tab=home (got ${aguiAlias.status} ${loc})`,
  );
...
console.log(
  "INTEGRATION-STACK: PASS — lobby, board, one-door AG-UI, MCP aligned.\n",
);
