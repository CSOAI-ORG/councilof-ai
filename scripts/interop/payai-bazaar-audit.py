#!/usr/bin/env python3
"""Audit our presence in the PayAI x402 Bazaar.

    python3 scripts/interop/payai-bazaar-audit.py            # writes docs/product/PAYAI-BAZAAR-AUDIT.md
    python3 scripts/interop/payai-bazaar-audit.py --json     # print the derived reading, write nothing
...
OUT = Path("docs/product/PAYAI-BAZAAR-AUDIT.md")
...
def main():
    ...
    a = ap.parse_args()
    try:
        items, total = scan(a.source)
    except (urllib.error.URLError, ValueError, KeyError) as e:
        print(f"UNCHECKABLE {type(e).__name__}: {e}; {OUT} left untouched", file=sys.stderr)
        return 2
    mine = ours(items)
    door_timeout = door_max_timeout()
    doc = {
        "kind": "csoai.payai-bazaar-audit/v0",
        "as_of": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "index": a.source,
        "declared_total": total,
        "scanned": len(items),
        "population_complete": len(items) >= total,
        "ours": [{"resource": m.get("resource"), "last_updated": m.get("last_updated") or m.get("lastUpdated"),
    ...
    lines = [f"# PayAI Bazaar — what the index holds for us", "",
             f"DERIVED by `scripts/interop/payai-bazaar-audit.py`. Never hand-edited; regenerate it.", "",
             f"- index: `{a.source}`",
             f"- scanned **{len(items)} of a declared {total}** — a complete population, which is what makes",
             f"  the absences below claims rather than guesses",
             f"- ours: **{len(mine)}**",
    ...
    if mine:
        ...
        lines.append("**Not listed.** No resource on councilof.ai or csoai.org appears in the index.")
    OUT.write_text("\n".join(lines) + "\n")
    print(f"wrote {OUT}: scanned {len(items)}/{total}, ours {len(mine)}")
    return 0
