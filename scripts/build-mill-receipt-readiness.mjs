...
for (const file of files) {
  ...
  if (card?.body?.signature_state !== "STAGED_UNSIGNED") continue;
  ...
  rows.push({
    id: card.id,
    card_url: `/interop/mill-cards-signed/${file}`,
...
if (counts.receipts !== 36 || counts.outer_signature_valid !== 36 ||
    counts.declared_staged_unsigned !== 36 || counts.regulatory_linked !== 5 ||
...
const document = {
  ...
  derived_from: "public/interop/mill-cards-signed/*.json where body.signature_state=STAGED_UNSIGNED",
...
if (process.argv.includes("--check")) {
  ...
  console.log(`mill receipt readiness PASS: ${counts.outer_signature_valid}/36 outer VALID; ${counts.regulatory_linked} linked; ${counts.regulatory_unlinked} unlinked`);
} else {
