...
Entropy accounting (honest arithmetic, not marketing):
  ...
  floor is 99 rolls. NOTE THE HONEST LIMIT: 99 rolls carry 255.91 bits, a
  hair UNDER 256. Strict >= 256 bits needs ceil(256 / log2(6)) = 100 rolls.
  The floor of 99 is the agreed ceremony policy (adequate for any realistic
  adversary); the exact bit count is printed every run so nobody reads
  "256" where 255.91 was delivered. Below the floor the tool FAILS CLOSED
  (exit 2) unless --allow-weak is passed, in which case every output is
  labelled WEAK and the run must never leave the test lane.
...
# Ceremony policy floor (see docstring): 99 rolls = 255.91 bits, honestly
# a hair under 256; 100 rolls = 258.50 bits clears it. 99 is the brief's
# floor; the exact figure is always printed.
MIN_ROLLS = 99
...
def main() -> int:
    ...
    ap.add_argument("--allow-weak", action="store_true",
                    help="TEST ONLY: proceed below the floor; output labelled WEAK")
    args = ap.parse_args()
    ...
    label = "WEAK — TEST ONLY" if weak else "OK"
    print(f"rolls: {n}")
    print(f"entropy_bits: {bits:.2f}  (log2(6)={BITS_PER_ROLL:.4f} bits/roll; "
          f"strict 256 needs 100 rolls; ceremony floor {args.min_rolls})")
