...
while true; do
  ...
  "$LOOPS/watchdog.sh" 8>&- >/dev/null 2>&1
  [ "$M" -ge 5 ] && "$LOOPS/root-check.sh" 8>&- >/dev/null 2>&1
  ...
  due() { [ "$H" = "$1" ] && [ "${M#0}" -ge "${2#0}" ]; }
  if due 03 00 && stamp bazaar-conformance; then nohup "$LOOPS/bazaar-conformance.sh" --now 8>&- >/dev/null 2>&1 & fi
  if due 03 30 && stamp settlement-dry;      then nohup "$LOOPS/settlement-dry.sh" --now 8>&- >/dev/null 2>&1 & fi
  if due 04 00 && stamp revenue-snapshot;    then nohup "$LOOPS/revenue-snapshot.sh" --now 8>&- >/dev/null 2>&1 & fi
  if due 05 00 && stamp hubcard-refresh;     then nohup "$LOOPS/hubcard-refresh.sh" --now 8>&- >/dev/null 2>&1 & fi
