...
def main() -> None:
    ...
    # Honesty rule (D24): image URLs must resolve; qualifying_models must be a
    # derived count or null (+note); status is "planned" until facts.json has a
    # live fact for the anchor. Badge images are per-card at /api/badge?card=<sha>.
    _IMG = "https://councilof.ai/api/badge?card=<signed-card-sha256>&subject=<url-encoded-subject>"
    _QNOTE = "no derivation exists; the previous value was the length of the probe candidate list"
    badges = {
        ...
        "badges": [
            {
                ...
                "qualifying_models": None,
                "qualifying_models_note": _QNOTE,
                "status": "active",
            },
            {
