...
export async function verifyToken(env: AuthEnv, token: string): Promise<{ sub: string; name: string } | null> {
  try {
    ...
    if (!ok) return null;
    const body = JSON.parse(parsed.payload) as {
      schema?: unknown;
      iss?: unknown;
      aud?: unknown;
      sub?: unknown;
    ...
    if (
      body.schema !== "csoai.auth-token/0.1" ||
      body.iss !== "https://councilof.ai" ||
      body.aud !== "council-os" ||
      ...
      body.iat > Date.now() + 60_000 ||
      body.exp <= Date.now() ||
      body.exp - body.iat > TOKEN_TTL_MS
    )
      return null;
