...
async function grounded(q: string, origin: string): Promise<string | null> {
  ...
  if (/\b(council os|ag.ui|agui|lobby|streaming|hitl|consent checkpoint)\b/i.test(q)) {
    return (
      `**Council OS** — governance router + measurement harness in one dock:\n\n` +
      `1. **Eunomia router** — /instruments — governance instruments (MCP/REST/AG-UI), not LLM proxies.\n` +
      `2. **Measurement harness** — GSPC board (GET /api/gspc), arena traces, RECEIPT-SPEC cards.\n` +
      `3. **AG-UI + grounded chat** — local pane commands, SSE when wired, or POST /api/chat.\n\n` +
      `Seeded prompts are typed, never auto-sent (consent lock). Open from the floating badge or header CTA.\n\n` +
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  if (env.SOV_GATE_URL && env.SOV_GATE_TOKEN) {
    try {
      ...
      // fall through to LiteLLM / grounded rather than return nothing
    }
  }

  // LiteLLM lane — OpenAI-compatible gateway (self-hosted OpenRouter substitute).
  if (env.LITELLM_PROXY_URL && env.LITELLM_MASTER_KEY) {
    try {
      const base = env.LITELLM_PROXY_URL.replace(/\/+$/, "");
      ...
      if (!r.ok) throw new Error("litellm HTTP " + r.status);
      const data: any = await r.json();
      const content = data?.choices?.[0]?.message?.content ?? "";
      if (content.trim()) {
        return reply(content, "council · litellm gateway · routed", "live");
      }
      throw new Error("empty answer from litellm");
    } catch {
