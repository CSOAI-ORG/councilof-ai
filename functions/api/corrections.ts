...
const LEDGER = {
  ...
  corrections: [
    // ── 2026-08-26: self-caught, and caused by one of our own guards ──────────
    {
      id: "C-2026-0826-13",
      date: "2026-08-26",
      what_was_wrong:
        "The estate published a manifest URL that returned 404, and a set of counts that nobody \u2014 including us \u2014 could re-derive. public/signed/chain.json, the card chain manifest listing all 335 positions, was deleted from the repository while every surface that depends on it kept publishing. GET /api/state went on serving card_chain.manifest = \"/signed/chain.json\" (confirmed 404 against a freshly booted local Pages server). /signed/chain-facts.json went on publishing 335 chain positions and 313 verified bodies while naming chain.json as the input it was derived from, so the derivation could not be reproduced by a reader or by us; scripts/derive-chain-facts.mjs simply could not run. The published verifier README instructed strangers to curl that exact URL, /signed/index.html had silently dropped the file from its artifact table, and two public pages \u2014 the honesty page and the home page \u2014 invited the reader to \"walk the positions yourself\" at the dead link, inside the very paragraph arguing that a complete-looking manifest must not do work a signature has not done. A count whose input is missing is exactly the defect this ledger exists to record, and we were committing it on the page where we describe it.",
      how_caught:
        "Self-caught during a 2026-08-26 audit reconciling the /signed/ surfaces after the deletion. The deletion was not a one-off: chain.json had been removed and restored five separate times by five separate lanes. The root cause was one of our own guards. scripts/signed-json-guard.mjs carried the header rule \"Do not claim 335\" \u2014 a rule about card_index.json, which is a deliberately CURATED SUBSET frozen at the verifiable floor of 150. Lanes read it as a blanket finding that the number 335 was dishonest wherever it appeared, and deleted a chain manifest that legitimately holds 335 positions. A guard whose comment is broader than its behaviour taught five readers a false rule.",
      fix:
        "chain.json restored and adjudicated against the bytes rather than against the commit message: 335 links with 335 distinct ids, the walk from head terminating exactly at genesis after 335 steps, all 313 body_published:true positions present under /signed/cards/, zero card files orphaned outside the chain, and all 313 Ed25519 signatures VALID under the pinned card-attestation key using the same verifier a stranger downloads. Re-running scripts/derive-chain-facts.mjs reproduced the committed chain-facts.json BYTE-IDENTICALLY, which establishes that the published counts had been honest all along and only their input had gone missing \u2014 so nothing was restated, only made checkable again. The 150-row curated index was not touched and remains at the floor its guards defend; the chain and the index are different sets and are now named as such. STRUCTURAL FIX, because a comment asking lanes not to do this had already failed five times: scripts/chain-manifest-guard.mjs now fails the build if any published surface advertises a /signed/ URL that does not resolve, or if the manifest disagrees with the bytes it indexes, and derive-chain-facts.mjs --check fails the build if the committed counts stop re-deriving. Both run in build:client. The misleading header in signed-json-guard.mjs is now scoped explicitly to card_index.json. /signed/HOW-TO-VERIFY.md gains a section 0 that tables the curated index against the chain manifest, and no longer offers the 150-row index count as the number of cards published.",
      status: "FIXED",
    },
