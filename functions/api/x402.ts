...
export const onRequestGet: PagesFunction<{
  ...
  const body = {
    ...
    one_line:
      "Verification is free forever. Agents pay per artefact: issuance, assembly, cadence — never a grade.",
    rail: {
      ...railMode(env),
      scheme: "exact",
      ...
      asset: {
        ...
        contract: USDC_BASE.asset,
        decimals: USDC_BASE.decimals,
      },
      pay_to: resolvePayTo(env),
      amounts:
        "only inside each resource's 402 challenge (accepts[].amount) — never on this catalog, never in prose",
    ...
    tiers: [
      {
        ...
        free_preview:
          u("/api/request-attestation?subject=<id>") +
          " (the 402 body carries csoai.preview: signed cards already on file)",
        deliverable:
          "one card-v0 leaf, surface ras.commission, ≤3KB payload, Ed25519 under did:web:csoai.org#board-attestation-1 when the Pages key is present (else sig_ed25519:null, declared)",
        never: [
          "a score",
          "a rank",
          "a certificate",
      ...
      },
      {
        ...
        deliverable:
          "OSCAL 1.1.0 assessment-results of already-signed cards (observations, relevant-to) + one manifest card-v0 (surface evidence.bundle) + the existing signed pack where one exists (/packs/eu-article-50)",
        never: [
      ...
      },
      {
        ...
        deliverable:
          "one feed document: signed signals index, signed First-Fine Watch, root.json, card index — each block with its published signature",
        never: ["scores as a product", "a ranking", "a rating"],
        also: {
          proof_bundle: u("/api/proof?bundle=1"),
          one_inclusion_free: u("/api/proof?sha=<64-hex>"),
        },
      },
      {
        tier: 1,
        ...
        free_preview:
          u("/api/rwa/evidence?asset=<symbol>&preview=1") +
          " (unsigned state, no raw-fetch hashes; symbols at /api/xrpl)",
        deliverable:
          "one canonical card-v0 leaf (public.notice / csoai.eater.xrpl-issuer/0.1 — the same schema as the free public-root leaf), ≤3072 bytes: AccountRoot flags, Domain, two-way TOML check, gateway_balances obligation, holders as the reader has them, per-fetch sha256 + inputs_sha256, Ed25519 under did:web:csoai.org#board-attestation-1 when the Pages key is present",
        never: [
          "a rating",
          "a guarantee",
          "a verdict",
      ...
      },
      {
        ...
        resource:
          u("/api/witness?sha256=<64-hex>&label=<text>") +
          "  |  " +
          u("/api/witness?url=<https public URL>") +
          "  |  POST " +
          u("/api/witness") +
          " (raw bytes ≤4 MiB, hashed then dropped)",
        free_preview:
          u("/api/witness?sha256=<64-hex>") +
          " (the 402 body carries csoai.preview: the digest, the fetch outcome, the TSA, the anchors, what happens)",
        free_status: u("/api/witness/status?sha256=<64-hex>"),
        deliverable:
          "one public.notice leaf (csoai.witness.hash/0.1) in the next hourly signed root under did:web:csoai.org#board-attestation-1 + an RFC-3161 timestamp reply over the digest from a documented public TSA + the ONE root's Rekor and OpenTimestamps anchors; queued in WITNESS_KV (503 NOT_YET when unbound — nothing charged)",
        attests:
        ...
        never: [
          ...
          "a bypass of a login, paywall, robots.txt or bot check (UNCHECKABLE, no charge)",
      ...
      },
      {
        ...
        free_preview:
          u("/api/feeds/provider-diff") +
          " (recent diffs + latest state per target, free; leaves in /feeds/provider-diff/leaves/)",
        deliverable:
          "every hash-only csoai.diff.provider-terms/0.1 leaf to date with its inclusion proof to the signed root, assembled; or a bespoke per-partner target list on the same method (GBP invoice: ?invoice=gbp&commissioned_by=<org>)",
        never: [
      ...
      },
      {
        ...
        free_preview:
          u("/api/receipts/batch?from=<iso>&to=<iso>&preview=1") +
          " (count, span, root count and the sha256 of the exact batch bytes — no leaves)",
        deliverable:
          "one canonical csoai.receipts.batch/0.1 document: every card-v0 leaf with as_of in the window (≤200), each with its Merkle inclusion path and the public root(s) that carried it (from /receipts/root-history.json — the git history of root.json indexed at build time), plus one signed manifest card-v0 (surface receipts.batch) citing the batch sha256",
        recent_free: [
          u("/root.json"),
          u("/cards/<sha16>.json"),
        ...
        honesty:
          "no settlement-receipt stream exists (/api/receipts/latest is UNPUBLISHED); these are measurement leaves, not payment receipts",
        never: [
      ...
      paid_tools: [
        {
          ...
          route: u("/api/request-attestation"),
          sells: "issuance",
        },
        {
          name: "art50_marking_evidence",
          route: u("/api/art50/marking-evidence"),
          sells: "issuance",
          note: "deployed; the tool still answers NOT_DEPLOYED on any origin where the route 404s",
        },
        {
          name: "rwa_evidence",
          route: u("/api/rwa/evidence"),
          sells: "issuance",
          note: "deployed; the tool still answers NOT_DEPLOYED on any origin where the route 404s",
        },
        {
    ...
    invariants: {
      measurement_not_certification:
        "CSOAI LTD (UK 16939677) is an independent measurement body. It issues measurements and signed attestations, never certificates of conformity.",
      never_a_grade:
        "No tier sells a grade, a score, a pass/fail verdict, or a place on the board.",
      recomputable_for_free:
        "Every artefact is independently recomputable; a buyer pays for issuance, assembly and a durable independent signature — not for the answer.",
      no_public_price: "Amounts appear only in a 402 challenge.",
      financial_firewall:
        "No token, credit, or cash-settled index. Attestation is not tokenisation and confers no ownership.",
    },
    lid: CSOAI_LID,
