...
export const onRequestGet: PagesFunction<{ X402_PAY_TO?: string; X402_FACILITATOR_URL?: string }> = async ({ request, env }) => {
  ...
  const body = {
    ...
    tiers: [
      {
        tier: 1,
        id: "issuance",
        name: "Commission an attestation receipt (request-attestation)",
      ...
      },
      {
        ...
        deliverable: "OSCAL 1.1.0 assessment-results of already-signed cards (observations, relevant-to) + one manifest card-v0 (surface evidence.bundle), Ed25519 when the Pages key is present and explicitly unsigned otherwise + the existing signed pack where one exists (/packs/eu-article-50)",
        never: ["a conformity determination", "satisfied/not-satisfied findings", "a certificate"],
      },
      {
        tier: 3,
        id: "data_feed",
        name: "Evidence data feed (assembly + cadence)",
        resource: u("/api/eunomia-data?feed=1"),
        free_preview: u("/api/eunomia-data"),
        deliverable: "one feed document: signals index, signed First-Fine Watch, root.json, and card index — each block preserving its published verification state; unsigned indexes stay labelled",
      ...
      },
      {
        ...
        name: "Provider document diff feed (historical evidence batch / bespoke partner feed)",
      ...
      },
      {
        ...
        deliverable: "one canonical csoai.receipts.batch/0.1 document: every card-v0 leaf with as_of in the window (≤200), each with its Merkle inclusion path and the public root(s) that carried it (from /receipts/root-history.json — the git history of root.json indexed at build time), plus one manifest card-v0 (surface receipts.batch) citing the batch sha256, Ed25519 when the Pages key is present and explicitly unsigned otherwise",
    ...
    invariants: {
      ...
      recomputable_for_free: "Every artefact is independently recomputable; a buyer pays for issuance and assembly with explicit verification state — not for the answer. A signature is included only when signing material is available.",
