...
async function grounded(q: string, origin: string): Promise<string | null> {
  ...
  for (const [k, rxs] of ART5_CUES) {
    if (rxs.every((rx) => rx.test(q))) {
      return (
        `That is prohibited under **EU AI Act Article 5(1)(${k})** - ${ART5[k]}.\n\n` +
        why(k) +
  ...
  if (
    /\b(pricing|plans?|how much|grade cost|is (it|verify|verification) free)\b/i.test(
      q,
    )
  ) {
    return `No SaaS tiers. Reading GET /api/gspc and verifying signed records are free. Commissioned receipts and evidence artefacts use the x402 challenge or an agreed invoice; payment never buys a score or ranking. Open /dashboard?tab=measured&task=pricing-overview to inspect the exact contract before authorising anything.\n\n_Grounded in the published free rail and x402 catalogue, not by a model._`;
  }

  if (
    /\b(in plain words|actually measure|what (do|does) (the )?(council|csoai)|difference between measur|one-paragraph summary)\b/i.test(
      q,
    ) ||
  ...
  const axisIntent =
    /\b(score|accuracy|axis|axes|bench|gspc|leader|wilson|macro f1|unparsed|measured on)\b/i.test(
      q,
  ...
  if (hit) {
    if (isJailAxis(hit)) {
      ...
      return (
        ...
        (typeof hit.accuracy === "number"
          ? `Leader accuracy ${Number(hit.accuracy).toFixed(3)}`
          : "No board-grade accuracy") +
        (hit.n ? ` at n=${hit.n}` : "") +
    ...
    return (
      `**${hit.axis}** (${hit.bench}) is **MEASURED**.\n\n` +
  ...
  if (/\bjail\b/i.test(q) && canon.jail && !axes.some(isJailAxis)) {
    ...
    return (
      ...
      (typeof j.accuracy === "number"
        ? `Leader accuracy ${Number(j.accuracy).toFixed(3)}`
        : "No board-grade accuracy") +
      (j.n ? ` at n=${j.n}` : "") +
  ...
  if (
    /\b(board|scoreboard|axes|axis|gspc|coverage|how many axes|walk me through|overview|live board|how many.*measured|measured of)\b/.test(
      t,
    ) &&
      ...
      (uAxes.length
        ? `\n\nThe ${uAxes.length} carrying no number — UNMEASURED is a published status, not an omission:\n` +
          uAxes
            .map(
              (a: any) =>
                `- **${a.axis}** — ${String(a.status ?? "UNMEASURED")}, no score on this stamp`,
            )
            .join("\n")
        : "") +
  ...
  if (
    /\b(method|how do you|unpars(?:ed|able|eable)|interval|wilson|grader|n *[>=]* *30)\b/.test(
      t,
    )
  ) {
  ...
  if (
    /^(hi|hey+|hello+|yo|gm|ga|ge|good (morning|afternoon|evening)|greetings|sup|howdy|hiya|heya|hej|hola)\b/i.test(
      t,
    ) ||
    t === "hi" ||
    t === "hello"
  ...
  if (
    /\b(who are you|what (is|are|s) (this|you|council|csoai|the council of ai)|what do you do|tell me about (council|csoai|this|you)|about (council|csoai|you)|explain (council|csoai|this)|are you (an? )?(ai|bot|chatbot))\b/i.test(
      t,
    )
  ) {
    return (
  ...
  if (
    /\b(what can i ask|what can you (do|answer|help)|^help$|how (do|does) (this|it|i) (work|use)|what (are my )?options|menu|get started|where do i start|what now)\b/i.test(
      t,
    ) ||
    t === "help"
  ) {
  ...
  if (
    /\b(thank|thanks|cheers|ta\b|appreciate|nice|cool|great|awesome|ok|okay|bye|goodbye|see ya)\b/i.test(
      t,
    ) &&
    t.length < 40
  ) {
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  const messages = Array.isArray(body.messages)
    ...
    : typeof body.prompt === "string"
      ...
      : typeof body.message === "string"
        ...
        : [];
  const model =
    typeof body.model === "string" ? body.model : "sov6-ethics-v3-light";
  if (!messages.length)
  ...
  if (g)
    return reply(
      g,
      "grounded in published measurement - deterministic - recomputable",
      "grounded",
    );
  ...
  if (env.SOV_GATE_URL && env.SOV_GATE_TOKEN) {
    try {
      const r = await fetch(
        env.SOV_GATE_URL.replace(/\/+$/, "") + "/api/chat",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + env.SOV_GATE_TOKEN,
          },
          body: JSON.stringify({
            model,
            messages,
