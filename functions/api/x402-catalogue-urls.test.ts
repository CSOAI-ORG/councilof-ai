 ...
 * 2026-09-05, one was not: `free_preview` for the evidence-bundle tier read
 * `?obligation=<id>&subject=<s>` and answered **404 unknown_obligation**, because `<id>` means a
 * MODEL id on the request-attestation tier ten lines above and an OBLIGATION id here. The same
 * ambiguity had already cost a 404 in /.well-known/x402.json earlier the same day.
 *
 * This reads the catalogue from the Function, so a tier added there is covered the moment it is
...
const catalogue = async () => {
  ...
  return (await r.json()) as { tiers: { id: string; resource?: string; free_preview?: string }[] };
};

describe("/api/x402 publishes URLs a buyer can actually use", () => {
  it("gives every tier a resource and a free_preview that are URLs and nothing else", async () => {
    const { tiers } = await catalogue();
  ...
  it("never uses the bare <id> placeholder where the handler wants an obligation", async () => {
    const { tiers } = await catalogue();
    const eb = tiers.find((t) => (t.resource ?? "").includes("/api/evidence-bundle"));
