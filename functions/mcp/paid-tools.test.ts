...
describe("/mcp tools/call — paid tools", () => {
  ...
  it("bad arguments are refused before any fetch; the free seven still dispatch to their own handler", async () => {
    ...
    const missing = await (await call(rpc("tools/call", { name: "commission_card", arguments: {} }))).json();
    expect(missing.result.structuredContent.status).toBe("BAD_ARGUMENTS");
  ...
  it("buildPaidRequest pins each tool to its declared route and never to a caller-supplied URL", () => {
    ...
    expect(buildPaidRequest("witness_hash", { sha256: "A".repeat(64), label: "hello" }, ORIGIN)).toEqual({ error: "unknown paid tool: witness_hash" });
