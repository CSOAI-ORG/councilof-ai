    ...
    async (u: string | URL | Request, init?: RequestInit) => {
      ...
      if (req.headers.get("x-payment") && opts.paidOk !== false) {
        return new Response(
          ...
          { status: 200, headers: { "x-payment-response": "c2V0dGxlZA==" } },
        );
      }
      const pr = {
        x402Version: 2,
...
describe("/mcp tools/list — seven free + five paid, catalogue free, nothing labelled safe", () => {
  ...
  it("the free seven definitions are byte-identical to what the stdio server reads (no drift)", async () => {
    ...
    expect(
      canonical.tools.some((t: { name: string }) => PAID_FIVE.includes(t.name)),
    ).toBe(false);
  });
...
describe("/mcp tools/call — paid tools", () => {
  ...
  it("never labels a successful free preview as paid delivery", async () => {
    vi.stubGlobal(
      "fetch",
      async () =>
        new Response(JSON.stringify({ count: 4, unsigned: true }), {
          status: 200,
        }),
    );
    const r = await (
    ...
    const sc = r.result.structuredContent;
    expect(sc.status).toBe("RUNTIME_OBSERVED_UNSIGNED_PREVIEW");
    expect(sc.signed).toBe(false);
    expect(sc.settled).toBe(false);
    expect(sc.nothing_charged).toBe(true);
    expect(sc.deliverable).toBeUndefined();
  });
  ...
  it("a route not on this origin yet answers NOT_DEPLOYED — nothing invented, nothing charged", async () => {
    ...
    for (const [name, args] of [
      ...
      const r = await (
        await call(rpc("tools/call", { name, arguments: args }))
      ).json();
  ...
  it("bad arguments are refused before any fetch; the free seven still dispatch to their own handler", async () => {
    ...
    const r = await (
      await call(rpc("tools/call", { name: "witness_hash", arguments: {} }))
    ).json();
  ...
  it("buildPaidRequest pins each tool to its declared route and never to a caller-supplied URL", () => {
    ...
    const w = buildPaidRequest(
      "witness_hash",
      { sha256: "A".repeat(64), label: "hello" },
      ORIGIN,
    );
    if ("req" in w)
      expect(new URL(w.req.url).searchParams.get("sha256")).toBe(
        "a".repeat(64),
      );
